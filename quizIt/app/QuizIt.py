...
class QuizIt:
    ...
    def home(self):
        ...
        self.img_label = Label(self.masterFrame, image=self.img,
                               background='white')  # Displaying the German Flag at the top of the Application
        self.img_label.pack()
        o = Label(self.masterFrame, background='white')  # Empty space, just for aesthetic reasons
        o.pack()
        self.mot_text = Label(self.masterFrame, text='Good luck learning the language.', background='white',
        ...
        self.what_form = Label(self.input_frame, text='How would you like to learn?     ', background='white',
                               font='Helvetica 10')
    ...
    def create_opts(self, n):
        ...
        while b_val < n:
            self.btn = Radiobutton(self.frame_opt, text="foo", variable=self.opt_selected, value=b_val + 1,
                                   background='white')
    ...
    def check(self):
        ...
        if self.a == 2:
            ...
            self.score_ans = Label(self.frame_score, text='Please select an option first', foreground='red',
            ...
            if self.a == 0:
                ...
                self.frame_score = Frame(self.frame_btn, background='white')
                self.frame_score.grid(row=1, columnspan=5)
                self.score_ans = Label(self.frame_score, text='Wrong', foreground='red', background='white',
                                       font='Helvetica 10 bold')
                self.score_ans.pack()
            elif self.a == 1:
                self.frame_score = Frame(self.frame_btn, background='white')
                self.frame_score.grid(row=1, columnspan=5)
                self.score_ans = Label(self.frame_score, text='Score', foreground='green', background='white',
                                       font='Helvetica 10 bold')
                self.score_ans.pack()
                if self.checked == 0:
                    self.score += 1
    ...
    def quitt(self):
        ...
        if self.end:
            ...
            sure = messagebox.askyesno("askyesno",
                                       "Your score won't be considered if you quit. Are you sure you want to quit?",
    ...
    def print_results(self):
        self.scores_users[
            f'{self.usr} on {datetime.now().strftime("%m/%d/%Y at %H:%M:%S")} scored'] = f'{self.score} out of {self.num_quess} in the form of {self.form}'
